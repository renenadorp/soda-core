...
class NetezzaDataSource(DataSource):
    TYPE = 'netezza'

    SCHEMA_CHECK_TYPES_MAPPING: dict = {
        "TEXT": ["character varying", "varchar", "string", "national character varying", "character varying", "CHARACTER VARYING(50)"],
        "NUMBER": ["integer", "int"],
        "FLOAT": ["decimal"],
    ...
    def connect(self):
        ...
        if isinstance(self.host, str) and len(self.host) > 0:
            self.logs.debug(
                f'Netezza connection properties: host="{self.host}", port="{self.port}", database="{self.database}", user="{self.username}", options="{options}", connection_timeout="{self.connection_timeout}"'
            )
            ...
            raise ConnectionError(f"Invalid netezza connection properties: invalid host: {self.host}")
    ...
    def default_casify_table_name(self, identifier: str) -> str:
        return identifier.upper() #Updated 20230613 by RND: Netezza tables in uppercase
