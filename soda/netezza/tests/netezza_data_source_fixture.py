...
class NetezzaDataSourceFixture(DataSourceFixture):
    def __init__(self, test_data_source: str):
        ...
        is_schema_reuse_disabled = os.getenv("NETEZZA_REUSE_SCHEMA", "").lower() == "disabled"
    ...
    def _build_configuration_dict(self, schema_name: str | None = None) -> dict:
        return {
            "data_source netezza": {
                "type": "netezza",
                #"host": "localhost",
                "host": os.getenv("NETEZZA_HOST", "sodasql"),
                "username": os.getenv("NETEZZA_USERNAME", "sodasql"),
                "password": os.getenv("NETEZZA_PASSWORD"),
                "database": os.getenv("NETEZZA_DATABASE", "sodasql"),
                "schema": schema_name if schema_name else os.getenv("NETEZZA_SCHEMA", "public"),
            }
        }
